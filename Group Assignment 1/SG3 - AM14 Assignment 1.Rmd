...
```{r plotting_total_return_indexes, fig.height=7, fig.width=7}

#for MSFT both with and without dividends
ggplot()+
  geom_line(data=MSFT,aes(x=DATE,y=TR_RET),color="red")+
  geom_line(data=MSFT,aes(x=DATE,y=TR_RETX),color="blue")+
...
ggplot()+
  geom_line(data=GE,aes(x=DATE,y=TR_RET),color="red")+
  geom_line(data=GE,aes(x=DATE,y=TR_RETX),color="blue")+
  theme_fivethirtyeight() + theme(axis.title=element_text())+
...
```{r normalizing_GE_price, fig.height=5, fig.width=5}
...
ggplot()+
  geom_line(data=GE,aes(x=DATE,y=PRC),color="red")+
  geom_line(data=GE,aes(x=DATE,y=PRC_norm),color="blue")+
  theme_fivethirtyeight() + theme(axis.title=element_text())+
...
MSFT <-MSFT %>% 
  mutate(LRET=log(1+RET)) %>% 
  mutate(TR_LRET = cumsum(1*(1+LRET)))
...
These distributions demonstrate how the logged returns of key assets such as MSFT and GE are more leptokurtic (with fatter tails) than the normal returns, and more negatively skewed. This is to be expected since taking the log of returns adjusts for the effect of a substantial number of positive outliers. Aside from kurtosis and skew (which is represented by the logged mean return being consistently smaller than the normal mean return), the variance of the normal returns and logged returns are similar: the logged return variance being only marginally smaller than the normal return variance, again due to mitgation of the effects of outliers at the positive extreme. 

```{r MSFT_returns_plots, fig.height=5, fig.width=5}

#plotting normal and log returns for MSFT
ggplot()+
  geom_line(data=MSFT,aes(x=DATE,y=RET),color="red")+
  geom_line(data=MSFT,aes(x=DATE,y=LRET),color="blue")+
  theme_fivethirtyeight() + theme(axis.title=element_text())+
...
```{r}
#Importing the daily price dataset
...
MSFT_daily <-PS1_Daily_longer %>% 
  group_by(TICKER) %>% 
  filter(TICKER=="MSFT") %>% 
  mutate(TR_RET_MSFT = cumprod(1*(1+RET)))

GE_daily <-PS1_Daily_longer %>% 
  group_by(TICKER) %>% 
  filter(TICKER=="GE") %>% 
  mutate(TR_RET_GE = cumprod(1*(1+RET)))

SP500_daily <-PS1_Daily_longer %>% 
  select(1, 3) %>%  #select DATE and SPRTRN
  distinct() %>% 
  mutate(total_return = cumprod(1*(1+SPRTRN)))
