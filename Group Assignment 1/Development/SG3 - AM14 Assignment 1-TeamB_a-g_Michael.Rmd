...
# Using dplyr to select the 3 stocks (MSFT, GE, JPM) for dates b/w period 2/1/2003 and 31/12/2014

Prices_daily_filtered_i <- Prices_daily %>% 
  select(-c("XOM", "INTC", "C")) %>% 
  filter(DATE >= "2003-01-02" & DATE <= "2014-12-31")

# Calculating the Mean, Variance and Pairwise Covariance for the three stocks

summary_stats_i <- Prices_daily_filtered_i %>% 
  summarise(mean_MSFT = mean(MSFT), var_MSFT = var(MSFT),
            mean_GE = mean(GE), var_GE = var(GE),
            mean_JPM = mean(JPM), var_JPM = var(JPM))

# MSFT has the lowest variance
# GE has the lowest mean
# JPM has the highest mean and highest variance

covariance_matrix_i <- var(Prices_daily_filtered_i
    %>% select(c("MSFT", "JPM", "GE")))

# We can see that JPM and MSFT still have the highest pairwise correlation across all 3 combinations
...
# Compare Stat Summary

library(ggtext)

## Combine data and add period
summary_stats_comb <- summary_stats %>% 
  rbind(summary_stats_i) %>% 
  mutate(period = c("period_1","period_2"))

#summary_stats_comb

## Create data frame
summary_stats_df <- data.frame("company" = c("MSFT","GE","JPM","MSFT","GE","JPM"),
                               "period" = c("Period 1", 
                                            "Period 2", 
                                            "Period 1", 
                                            "Period 2", 
                                            "Period 1", 
                                            "Period 2"),
                              "mean" = c(summary_stats_comb$mean_MSFT[1],
                  summary_stats_comb$mean_GE[1],
                  summary_stats_comb$mean_JPM[1],
                  summary_stats_comb$mean_MSFT[2],
                  summary_stats_comb$mean_GE[2],
                  summary_stats_comb$mean_JPM[2]),
                              "var" = c(summary_stats_comb$var_MSFT[1],
                  summary_stats_comb$var_GE[1],
                  summary_stats_comb$var_JPM[1],
                  summary_stats_comb$var_MSFT[2],
                  summary_stats_comb$var_GE[2],
                  summary_stats_comb$var_JPM[2]))
...
var_plot <- summary_stats_df %>% # Use summary data
  ggplot(aes(x = company, y = var, fill = company)) + # Show Variance for 3 Assets on Y
  geom_col() + # Create column graph
  facet_wrap(~ period) + # Divide graph by Time Frames to show difference
  theme_fivethirtyeight() +
  labs(title = "Variance of the Three Assets in Period 1 & 2", 
       subtitle = "<b style='color:#00BA38'>J.P. Morgan</b> <b>has the highest variance in period 1 but<br><b style='color:#F8766D'>General Electric</b> is by far the highest in period 2</b>") +
  theme(plot.subtitle = element_markdown(),
...
mean_plot <- summary_stats_df %>% # Use summary data
  ggplot(aes(x = company, y = mean, fill = company)) + # Show Mean for 3 Assets on Y
  geom_col() + # Create column graph
  facet_wrap(~ period) + # Divide graph by Time Frames to show difference
  theme_fivethirtyeight() +
  labs(title = "Mean of the Three Assets in Period 1 & 2", 
       subtitle = "<b style='color:#619CFF'>Microsoft</b> <b>has the highest mean in period 1 but lowest in period 2 with<br><b style='color:#F8766D'>General Electric</b> scoring highest there</b>") +
  theme(plot.subtitle = element_markdown(),
        legend.position = "none",
        strip.background = element_rect(
          color="#3C3C3C", 
...
## Fix table for graph
cov_mat_1_rs <- melt(covariance_matrix)
cov_mat_2_rs <- melt(covariance_matrix_i)

## Grey out variance
cov_mat_1_rs <- cov_mat_1_rs %>% 
  mutate(value = ifelse(Var1 == Var2, NA, value)) # Set the "self covariance" to NA
  
cov_mat_2_rs <- cov_mat_2_rs %>% 
  mutate(value = ifelse(Var1 == Var2, NA, value)) # Set the "self covariance" to NA

## Create Plots

### Period 1
cov_mat_plot_1 <- cov_mat_1_rs %>% 
  ggplot(aes(x = Var1, y = Var2, fill = value)) + 
  geom_tile() +
  scale_fill_gradient(na.value = "black", low = "blue", high = "red") +
  theme_fivethirtyeight() +
...
## Show Plots
plot_merge <- cov_mat_plot_1 + cov_mat_plot_2

plot_merge +
  plot_annotation(
    title = "Covariance of the Three Assets in Period 1 & 2",
    subtitle = "While J.P.Morgan and Microsoft have high covariance in both periods,\nthe covariance between Microsoft and GE changes place with J.P.Morgan and GE in Period 2") +
  theme(plot.subtitle = element_markdown(),
        strip.background = element_rect(
          color="#3C3C3C", 
          fill="#F0F0F0",
